...
# Loading counts, info, and getter functions
load("data/OogCounts.RData")

# Tarikere et al. did three replicates at three timepoints in oogenesis during
# larval development and in three tissues: Germ, Somatic, and Whole Ovary.

# For now, we will cluster the combined three tissues, but we could return
# To cluster just one tissue if needed
# For the repsplit, reps1 go to split A, reps3 go to split B, and the 9
# observations in reps2 will the split randomly between A and B

#### Checking for 0-count genes ####
# removing 0-count genes in split counts (which will also remove 
# 0-count genes in full count matrix)

# splitting counts
set.seed(23)
colnamesA_idxs <- which(getReplicate(colnames(counts)[-1]) == 1)
colnamesB_idxs <- which(getReplicate(colnames(counts)[-1]) == 3)
rep2_idxs <- which(getReplicate(colnames(counts)[-1]) == 2)
rep2_A_idxs <- sample(rep2_idxs, size = 5, replace = FALSE)
rep2_B_idxs <- setdiff(rep2_idxs, rep2_A_idxs)
colnamesA_idxs <- c(colnamesA_idxs, rep2_A_idxs)
colnamesB_idxs <- c(colnamesB_idxs, rep2_B_idxs)
...
sum(rowSums(countsB[,-1]) == 0) # if not, don't need to remove. If yes, try re-running partition
...
plotdf <- tibble(sample = colnames(counts)[-1],
                 ...
                 cell_type = getCellType(colnames(counts)[-1]),
...
# As seen in the PCA, Germ is the noisiest. None of these genes have much
# variability. Cactus possibly spiked in germ tissue before 72hr AEL

#### Smoothing by spline ####
deg_free <- 7

# version that smooths across all three cell types
smoothCountMatrix <- function(.cts_x, .cts_y, .df = deg_free) {
  ngenes <- nrow(.cts_x)
  cellhour_vec_x <- paste(getCellType(colnames(.cts_x)),
                          getHour(colnames(.cts_x)), sep = "_")
  cellhour_vec_y <- paste(getCellType(colnames(.cts_y)),
                           getHour(colnames(.cts_y)), sep = "_")
  all_cellhours <- union(cellhour_vec_x, cellhour_vec_y)
  outlist <- map(c(1:ngenes), \(g) {
    ...
    gene_vec <- purrr::map(all_cellhours, \(hr) {
      if (hr %in% cellhour_vec_x & hr %in% cellhour_vec_y) {
        g_hr_x <- .cts_x[g, cellhour_vec_x == hr] |> as.numeric() # note there might be 2 replicates in x or in y
        g_hr_y <- .cts_y[g, cellhour_vec_y == hr] |> as.numeric()
        return(mean(c(g_hr_x, g_hr_y)))
      }
      if (hr %in% cellhour_vec_x) {
        g_hr_x <- .cts_x[g, cellhour_vec_x == hr] |> as.numeric()
        return(mean(c(g_hr_x)))
      }
      if (hr %in% cellhour_vec_y) {
        g_hr_y <- .cts_y[g, cellhour_vec_y == hr] |> as.numeric()
        return(mean(c(g_hr_y)))
      }
    }) |> unlist()
    smooth_y <- smooth.spline(x = gene_vec,
                              df = .df,
                              keep.stuff = TRUE)
    return(smooth_y$y)
  })
  ...
  colnames(outmat) <- all_cellhours
  return(outmat)
}

# tests for smoothCountMatrix
colnames1_idxs <- colnamesA
colnames2_idxs <- colnamesB
gene_idx <- creba_idx
...
plotdf <- bind_rows(tibble(status = "before1",
                           ...
                           cellhour = paste(getCellType(colnames(test_counts_1)),
                                            getHour(colnames(test_counts_1)), sep = "_")),
                    tibble(status = "before2",
                           expr = as.numeric(test_counts_2),
                           cellhour = paste(getCellType(colnames(test_counts_2)),
                                            getHour(colnames(test_counts_2)), sep = "_")),
                    tibble(status = "after",
                           expr = as.numeric(test_smooth),
                           cellhour = colnames(test_smooth)))
ggplot(plotdf, aes(x = cellhour, y = expr)) +
...
# smoothing
deg_free
smoothA <- smoothCountMatrix(.cts_x = counts[,colnamesA],
                             .cts_y = counts[,colnamesA], # not a mistake using the same split twice
                             .df = deg_free)
smoothA <- bind_cols(counts$gene_name, smoothA)
colnames(smoothA)[1] <- "gene_name"
smoothB <-smoothCountMatrix(.cts_x = counts[,colnamesB],
                            .cts_y = counts[,colnamesB],
                            .df = deg_free)
smoothB <- bind_cols(counts$gene_name, smoothB)
colnames(smoothB)[1] <- "gene_name"
smooth_full <- cbind(smoothA, smoothB[,-1])
colnames(smooth_full) <- c("gene_name", paste0(colnames(smoothA)[-1], "_A"),
                           paste0(colnames(smoothB)[-1], "_B"))
#### Clustering ####
# @input: counts matrix
...
plotModuleExpressionShapes <- function(.ME_object,
  ...
  plotdf <- expr_mat |>
    mutate(sample_name = rownames(expr_mat)) |> 
    pivot_longer(cols = colnames(expr_mat),
                 names_to = "label",
                 values_to = "expr")
  if (.separate_replicates) {
    plotdf$replicate <- getReplicate(plotdf$sample_name)
  }
  plotdf$hour <- getHour(plotdf$sample_name)
  plotdf$cell_type <- getCellType(plotdf$sample_name)
  p <- ggplot(plotdf, aes(x = interaction(hour, cell_type), y = expr)) +
...
rownames(AvgExpr_A) <- paste(getCellType(rownames(AvgExpr_A)),
                             getHour(rownames(AvgExpr_A)), sep = "_")
colnames(AvgExpr_A) <- paste0(colnames(AvgExpr_A), "_A")
AvgExpr_B <- MEs_B$averageExpr
rownames(AvgExpr_B) <- paste(getCellType(rownames(AvgExpr_B)),
                             getHour(rownames(AvgExpr_B)), sep = "_")
...
getDist(.clust1 = "AE1_A", .clust2 = "AE2_B", .pca = pca_AB, .nPCs = nPCs) # should be small
getDist(.clust1 = "AE7_A", .clust2 = "AE8_B", .pca = pca_AB, .nPCs = nPCs) # should be small
getDist(.clust1 = "AE13_A", .clust2 = "AE12_B", .pca = pca_AB, .nPCs = nPCs) # should be small
getDist(.clust1 = "AE8_A", .clust2 = "AE14_B", .pca = pca_AB, .nPCs = nPCs) # should be large
...
# A PCA vs A WGCNA
ggarrange(p_A_PCA, p_A_WGCNA, nrow = 1, ncol = 2, legend = "none")
# B PCA vs B WGCNA
ggarrange(p_B_PCA, p_B_WGCNA, nrow = 1, ncol = 2, legend = "none")
...
oldMEs <- moduleEigengenes(t(counts[,-1]), colors = clusters$df$label)
# pre-merge:
plotModuleExpressionShapes(oldMEs, .MEs_or_AvgExpr = "AvgExpr",
                           .separate_replicates = TRUE) # pre-merge
# WGCNA::mergeCloseModules:
plotModuleExpressionShapes(newMEs_WGCNA, .MEs_or_AvgExpr = "AvgExpr",
                           .separate_replicates = TRUE) # postmerge, WGCNA
# PCA-distance merge:
plotModuleExpressionShapes(newMEs_PCA, .MEs_or_AvgExpr = "AvgExpr",
                           .separate_replicates = TRUE) # postmerge, PCA
# conclusion: WGCNA seems better
...
table(clustdf$robust, clustdf$label) # 4 and 5 are the least represented
...
#### Saving ####
save(robust_cluster_genes, clustdf, counts, infodf, file = "data/ClusteringOog.RData")
